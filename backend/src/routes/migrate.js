...
router.get('/bundle', (req, res) => {
  ...
  try {
    ...
    out.transaction(() => rows.forEach(r => ins.run(r.id, r.email, r.type, r.color, r.config, r.created_at ?? 0)))()
    out.close()
...
router.post('/bundle', async (req, res) => {
  ...
  try {
    ...
    if (fs.existsSync(accountsDb)) {
      ...
      db.transaction(() => rows.forEach(r => ins.run(r.id, r.email, r.type, r.color, r.config, r.created_at ?? Math.floor(Date.now() / 1000))))()
