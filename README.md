...
### v5.3.2
- 修复 IMAP IDLE 无限重试问题：永久性错误（黑名单/密码错误）立即停止重试，超时类错误指数退避（30s→60s→120s→240s→480s），最多重试5次
- 修复内存泄漏：去掉 Outlook autoSync 里多余的 IMAP 连接调用
- autoSync 间隔从60秒改为5分钟，降低服务器负担
- 加内存守护：RSS 超 1.2GB 自动重启防止泄漏累积
