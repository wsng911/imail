...
export default function ComposePanel({ accounts, initial = {}, onClose }: Props) {
  ...
  // 回复时 initial.body 是 HTML 引用块，新内容单独输入
  const isReply = initial.isReply ?? false
  const rawBody = initial.body ?? ''
  // 草稿 body 可能是 HTML，用 DOMParser 转纯文本
  const plainBody = isReply ? '' : (() => {
    try {
      const doc = new DOMParser().parseFromString(rawBody, 'text/html')
      return doc.body.textContent?.trim() || rawBody
    } catch { return rawBody }
  })()
  const [replyText, setReplyText] = useState(plainBody)
  ...
  async function handleSend() {
    ...
    setSending(true)
    setError('')
    const finalBody = isReply
      ? `<div>${replyText.replace(/\n/g, '<br>')}</div>${initial.body ?? ''}`
      : replyText
    try {
      const res = await fetch('/api/emails/send', {
        ...
        credentials: 'include',
        body: JSON.stringify({ fromAccountId: fromId, to, bcc: bcc || undefined, subject, body: finalBody }),
      })
      if (!res.ok) {
        ...
        if (initial.draftId) {
          await fetch(`/api/emails/${initial.draftId}`, { method: 'DELETE', credentials: 'include' })
        }
        onClose()
      }
    } catch {
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
        <span className="text-sm font-semibold text-gray-900">{isReply ? '回复邮件' : '新邮件'}</span>
        <button onClick={onClose}><X size={20} className="text-gray-500" /></button>
      </div>

      <div className="flex flex-col overflow-y-auto" style={{ flex: '0 0 auto' }}>
        {/* From */}
      ...
      <textarea
        ...
        style={{ minHeight: isReply ? 120 : undefined, flex: isReply ? '0 0 auto' : 1 }}
      />
      {isReply && initial.body && (
        <div className="flex-1 overflow-y-auto px-4 py-2 border-t border-gray-100 text-sm text-gray-500"
      ...
      <div className="px-4 py-3 border-t border-gray-100 flex items-center justify-between">
        ...
        <button
          onClick={handleSend} disabled={sending || !fromId}
